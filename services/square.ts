...
export async function fetchChannels() {
  return supabase.from("channels").select("*").order("category").order("created_at");
}
...
export async function uploadPostImage(userId: string, file: File) {
  ...
  const { error } = await supabase.storage.from("post-images").upload(imagePath, file, {
...
export async function deleteNotification(notificationId: string) {
  return supabase.from("notifications").delete({ count: "exact" }).eq("id", notificationId);
}
