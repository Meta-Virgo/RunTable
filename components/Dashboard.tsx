  ...
  return (
    <div className="flex-1 overflow-y-auto p-4 md:p-8 custom-scrollbar animate-fade-in">
        <div className="max-w-6xl mx-auto space-y-8 md:space-y-12 pb-12">
           ...
           {isKP && (
               <section>
                    <div onClick={onEditModule} className="group glass-panel bg-slate-800/40 rounded-2xl p-6 md:p-8 relative overflow-hidden transition-all cursor-pointer hover:bg-slate-800/60">
               ...
               <section>
                   ...
                   <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
                     {npcCharacters.map(npc => { 
                       ...
                       return (
                         <div key={npc.id} onClick={() => onEditChar(npc)} className={cn("glass-panel bg-slate-800/30 p-5 md:p-6 rounded-2xl relative overflow-hidden group border-t-2 transition-all cursor-pointer hover:bg-slate-800/50", isMonster ? "border-t-rose-500/50" : "border-t-cyan-500/50")}>
                           <div className="flex items-start gap-4 mb-4 md:mb-6">
               ...
               <section className="pt-8 border-t border-white/5">
                    ...
                    <div className="glass-panel bg-red-900/10 border-red-500/20 p-6 md:p-8 rounded-2xl flex flex-col md:flex-row justify-between items-center gap-6">
                        <div className="flex-1">
                            ...
                            <p className="text-slate-400 text-sm leading-relaxed">这将永久删除该房间及其所有数据。此操作不可恢复。</p>
                        </div>
                        <div className="flex items-center gap-4 shrink-0">
                            {resetConfirm ? (
                                <>
                                    <div className="flex flex-col items-end gap-1">
                                        <Button onClick={() => { onDeleteRoom(); setResetConfirm(false); }} variant="dangerActive" icon={AlertTriangle} size="lg">确认删除</Button>
                                ...
                                <Button onClick={() => setResetConfirm(true)} variant="danger" icon={AlertTriangle} size="lg">删除房间</Button>
                            )}
                        </div>
                    </div>
               </section>
