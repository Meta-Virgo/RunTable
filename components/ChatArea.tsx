...
interface ChatAreaProps {
  ...
  onRollDice: (count: number, type: number, isSecret: boolean) => void;
  onShowStory: () => void;
  isKP: boolean;
  kpId: string | null;
}
      ...
      <div className="flex-1 overflow-y-auto px-3 md:px-8 py-4 md:py-6 space-y-6 custom-scrollbar">
        ...
        {logs.map((log) => {
            ...
            if (isDice) {
                  ...
                  return (
                    <div key={log.id} className="flex flex-col items-center py-4 animate-slide-up">
                        <div className={cn("relative overflow-hidden bg-slate-900 rounded-xl border-l-4 shadow-2xl p-4 min-w-[260px] max-w-sm w-full", isHidden ? "border-purple-500" : "border-indigo-500")}>
                            {isHidden ? <EyeOff className="absolute -right-4 -bottom-4 text-purple-500/10 w-32 h-32 transform rotate-12" /> : <Dice5 className="absolute -right-4 -bottom-4 text-indigo-500/10 w-32 h-32 transform rotate-12" />}
                            <div className="flex justify-between items-center mb-2 relative z-10">
                                <div className="flex flex-col">
                                    <span className="text-sm font-bold text-white">{displayName}</span>
                                    <span className={cn("text-[10px] font-bold tracking-widest uppercase opacity-70", isHidden ? "text-purple-400" : "text-indigo-400")}>{isHidden ? "Secret Roll" : "Dice Roll"}</span>
                                </div>
                                <span className="text-xs text-slate-500 font-mono">{log.timestamp}</span>
                            </div>
                            <div className="flex items-baseline gap-3 relative z-10"><span className="text-slate-400 text-sm font-medium">投掷了</span><span className="text-white text-lg font-bold">{diceData.count}D{diceData.type||6}</span><span className="text-slate-600">:</span><span className={cn("text-4xl font-black font-mono", isHidden ? "text-purple-400" : "text-indigo-400")}>{diceData.total}</span></div>
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        {isKP && (
                            <button 
                                ...
                                className={cn(
                                    "p-2 md:p-1.5 rounded-xl transition-colors h-10 w-10 flex items-center justify-center border",
                        ...
                        <button onClick={() => onRollDice(diceCount, diceType, isSecret)} className="p-2 md:p-1.5 hover:bg-slate-800 rounded-xl text-slate-400 hover:text-indigo-400 transition-colors h-10 w-10 flex items-center justify-center border border-transparent hover:border-slate-700" title="投掷"><Dice5 size={20} /></button>
                    </div>
                    {isKP && <button onClick={onShowStory} className="p-2 text-slate-500 hover:text-slate-300 transition-colors hover:bg-white/5 rounded-lg" title="战报预览"><FileText size={18} /></button>}
                </div>
