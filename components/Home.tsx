...
const RoomCard: React.FC<RoomCardProps> = ({ room, currentUserId, myCharacters, onJoinRoom }) => {
        ...
        <div className="bg-slate-800/30 border border-slate-700/50 hover:border-indigo-500/30 rounded-xl p-5 transition-all hover:bg-slate-800/50 group flex flex-col">
            ...
            <div className="pt-4 border-t border-white/5 space-y-3">
                <div className="flex flex-col gap-2">
                    <label className="text-xs text-slate-500 font-medium">选择角色加入:</label>
                    <select 
...
export const Home: React.FC<HomeProps> = ({ onJoinRoom, onLogout }) => {
    const [activeTab, setActiveTab] = useState<'rooms' | 'characters' | 'profile'>('rooms');
    ...
    useEffect(() => {
        ...
        supabase.auth.getUser().then(async ({ data: { user } }) => {
            if (user) {
                ...
                // Fetch User Profile
                const { data: profile } = await supabase.from('profiles').select('user_code, nickname, bio, created_at').eq('id', user.id).single();
                if (profile) {
                    ...
                    const { data: newProfile} = await supabase
                        .from('profiles')
                        .insert({ id: user.id, nickname: user.email?.split('@')[0] || 'User' })
                        .select()
                        .single();
                    if (newProfile) {
                    ...
                    setRooms(prev => prev.filter(r => r.id !== payload.old.id));
                     ...
                     if (updatedRoom.status !== 'open') {
                         ...
                         setRooms(prev => {
                             ...
                             if (exists) return prev.map(r => r.id === updatedRoom.id ? updatedRoom : r);
    ...
    const handleSaveCharacter = async (char: Character) => {
        ...
        if (editingChar) {
            const { error } = await supabase
                ...
                .eq('id', char.id);
            if (!error) {
                setMyCharacters(prev => prev.map(c => c.id === char.id ? char : c));
        ...
        <div className="min-h-screen bg-[#020617] text-slate-200 flex flex-col font-sans">
                ...
                <div className="flex items-center gap-4">
                    ...
                    <nav className="hidden md:flex bg-slate-800/50 p-1 rounded-lg">
                        <button 
                            ...
                            className={`px-4 py-1.5 rounded-md text-sm font-medium transition-all ${activeTab === 'rooms' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-400 hover:text-white'}`}
                        >
                            大厅
                        </button>
                        <button 
                            onClick={() => setActiveTab('characters')}
                            className={`px-4 py-1.5 rounded-md text-sm font-medium transition-all ${activeTab === 'characters' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-400 hover:text-white'}`}
                        >
                            车卡
                        </button>
                        <button 
                            onClick={() => setActiveTab('profile')}
                            className={`px-4 py-1.5 rounded-md text-sm font-medium transition-all ${activeTab === 'profile' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-400 hover:text-white'}`}
                        >
                            我的
                        </button>
                    </nav>
             ...
             <main className="flex-1 container mx-auto p-4 md:p-8 max-w-6xl">
                 ...
                 <div className="md:hidden flex bg-slate-800/50 p-1 rounded-lg mb-6">
                    ...
                    <button onClick={() => setActiveTab('characters')} className={`flex-1 py-2 rounded-md text-sm font-medium ${activeTab === 'characters' ? 'bg-indigo-600 text-white' : 'text-slate-400'}`}>车卡</button>
                    <button onClick={() => setActiveTab('profile')} className={`flex-1 py-2 rounded-md text-sm font-medium ${activeTab === 'profile' ? 'bg-indigo-600 text-white' : 'text-slate-400'}`}>我的</button>
                 </div>
                 ...
                 {activeTab === 'rooms' ? (
                     <div className="space-y-6">
                         ...
                         <div className="flex flex-col md:flex-row gap-4 justify-between items-center">
                             ...
                             <Button icon={Plus} onClick={() => setShowCreateRoom(true)}>创建房间</Button>
                         </div>
                     ...
                     <div className="space-y-6">
                         <div className="flex justify-between items-center">
                             ...
                             <Button icon={Plus} onClick={() => { setEditingChar(null); setShowCharModal(true); }}>新建角色</Button>
                         </div>
                         
                         <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                             {myCharacters.map(char => (
                                 <div key={char.id} className="bg-slate-800/30 border border-slate-700/50 hover:border-indigo-500/30 rounded-xl p-5 transition-all group relative overflow-hidden">
                                     ...
                                     <div className="grid grid-cols-4 gap-2 mb-4 pl-3">
                                         {['STR','DEX','POW','APP'].map((stat, i) => (
                                             <div key={stat} className="bg-slate-900/50 rounded p-1.5 text-center">
                                                 <div className="text-[10px] text-slate-500 font-bold">{stat}</div>
                                                 <div className="text-sm font-mono text-slate-300">
                                                     {[char.str, char.dex, char.pow, char.app][i]}
                                                 </div>
                                             </div>
                                         ))}
                                     </div>
                                     <div className="pl-3">
                                         <Button variant="secondary" size="sm" className="w-full" onClick={() => { setEditingChar(char); setShowCharModal(true); }}>编辑档案</Button>
                                     </div>
                                 </div>
                             ))}
                       ...
                       <div className="bg-slate-800/30 border border-slate-700/50 rounded-2xl p-8 text-center relative overflow-hidden">
                           ...
                           {!isEditingProfile ? (
                               <>
                                   ...
                                   <div className="w-24 h-24 bg-slate-900 rounded-full mx-auto mb-4 flex items-center justify-center border-2 border-slate-700 shadow-xl">
                                       <User size={48} className="text-slate-500" />
                                   </div>
                                   <h2 className="text-2xl font-bold text-white mb-1">{userNickname || '未命名用户'}</h2>
                                   <div className="flex justify-center items-center gap-2 mb-4">
                                       <span className="text-sm text-slate-400 font-mono bg-slate-900/50 px-2 py-1 rounded">UID: {userCode || '---'}</span>
                                   </div>
                                   {userBio && (
                                       <p className="text-slate-300 mb-6 max-w-md mx-auto italic">"{userBio}"</p>
                                   )}
                                   ...
                                   <div className="grid grid-cols-2 gap-4 text-left mt-6">
                                       <div className="bg-slate-900/50 p-4 rounded-xl border border-slate-700/30">
                                           <div className="text-xs text-slate-500 uppercase font-bold mb-1">我的车卡</div>
                                           <div className="text-2xl font-mono font-bold text-indigo-400">{myCharacters.length}</div>
                                       </div>
                                       <div className="bg-slate-900/50 p-4 rounded-xl border border-slate-700/30">
                                           <div className="text-xs text-slate-500 uppercase font-bold mb-1">注册时间</div>
                               ...
                               <div className="max-w-md mx-auto space-y-4 text-left">
                                   <h3 className="text-lg font-bold text-white mb-4 text-center">编辑个人资料</h3>
                                   <Input label="昵称" value={editNickname} onChange={e => setEditNickname(e.target.value)} />
                                   <Textarea label="个人简介" value={editBio} onChange={e => setEditBio(e.target.value)} />
                                   <div className="flex justify-center gap-3 pt-2">
                                       <Button variant="ghost" onClick={() => setIsEditingProfile(false)}>取消</Button>
                                       <Button onClick={handleUpdateProfile} disabled={loading}>{loading ? <Loader2 className="animate-spin" /> : '保存更改'}</Button>
                                   </div>
                               </div>
                           )}
                       </div>
             ...
             {showCharModal && (
                 <CharacterModal 
                    ...
                    onDelete={editingChar ? () => handleDeleteCharacter(editingChar.id) : undefined}
