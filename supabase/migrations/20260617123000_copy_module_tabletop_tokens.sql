          ...
          (
            select coalesce(
              jsonb_agg(
                ...
                || jsonb_build_object(
                  'roomId', p_room_id::text,
                  'sceneId', v_new_scene_id::text,
                  'characterId', mapped_character_id::text,
                  'updatedAt', timezone('utc'::text, now())::text
                )
              ),
              '[]'::jsonb
            ...
            from (
              select
                token,
                nullif(
                  v_character_map->>replace(coalesce(token->>'characterId', ''), 'module-character:', ''),
                  ''
                )::uuid as mapped_character_id
              from jsonb_array_elements(coalesce(v_scene.tabletop_state->'tokens', '[]'::jsonb)) token
            ) token_map
            where mapped_character_id is not null
          ),
        true
      );
