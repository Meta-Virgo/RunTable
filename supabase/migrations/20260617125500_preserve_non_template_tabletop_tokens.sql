...
begin
  ...
  if v_has_tabletop_state and v_tabletop_state is not null then
    ...
    delete from public.room_tabletop_tokens existing_token
    where existing_token.room_id = p_room_id
      and exists (
        select 1
        ...
        where nullif(token->>'sceneId', '') is not null
          ...
          and existing_token.scene_id = (token->>'sceneId')::uuid
          and existing_token.character_id = (token->>'characterId')::uuid
      );
